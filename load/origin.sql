-- staging to consumption (Origin)
-- TODO: externalise / calculate the start date
-- TODO: check that the days inserted don't already exist
insert into consumption (client_id,day,prd,kwh)
select 
1 as client_id,
(date '13/03/2013'-date '01/01/2010') + (t.id-(select min(id) from staging_origin1))/48+1 as day,
(case (time)
