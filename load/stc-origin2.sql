...
((case when (length(dt)=7) then to_date('0'||dt,'DD/MM/YY') when (length(dt)=8) then to_date(dt,'DD/MM/YY') when (length(dt)=10) then to_date(dt,'DD/MM/YYYY') else to_date(dt,'DD/Mon/YYYY') end)-date '01/01/2010')+1 as day,
p as prd,
k as kwh
from
(
